	...
	function doAction(fn,id){
		if(fn=='del'){
			...
			dialog.confirm(function() {
				 $.ajax({
					 type:"post",  
		                url:ctx+"/base/positionActionDel.htm",  
		                data:{recordId:id},  
		                success:function(data){ 
		    	    		...
		    	    		if(data==true){
		    	    			...
		    	    			new DialogFunc("通知", '删除失败,该岗位已经被使用!', "alert").init(
		    							'alert', 'hiddenDialog').show();
		    	    		}
		                },  
		                error:function(e) {
			...
			$.post(ctx+'/base/positionActionInput.htm', {recordId:id}, function(data) {
				$("#position_input").html(data);
			});
		}	
	}
