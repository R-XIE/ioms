...
$(function() {
	getBootstrapPaginator(backupLocal);
	$('#backupDateBE').daterangepicker(dataParam);
});
function doAction(fn, id) {
	if (fn == 'del') {
		...
		dialog.confirm(function() {
			$.ajax({
				...
				success : function(data) {
					...
					if (data == true) {
						new DialogFunc("通知", '备份记录删除成功!', "alert").init('alert',
								'hiddenDialog').show();
						$('#index_refresh').trigger('click');

					} else {
						new DialogFunc("通知", '备份记录删除失败!', "alert").init(
		...
		blockPost(ctx + backupLocal + 'Input.htm');
	} else if (fn == 'edit') {
		blockPost(ctx + backupLocal + 'Input.htm', {
			recordId : id
		});
	} else if (fn == 'view') {
		blockPost(ctx + backupLocal + 'View.htm', {
			recordId : id
		});
	} else if (fn == 'reset') {
		resetCondition();
		...
		blockPost(ctx + backupLocal + 'ListAjax.htm', param);
	}
}
function ck_index() {
